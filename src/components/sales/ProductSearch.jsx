...
export default function ProductSearch({ products, onAdd }) {
  ...
  const handleSelect = (product) => {
    onAdd({ 
        kind: product.category === "FRAMES" ? "LENSES" : "MEDICATION", 
  ...
  return (
    <div style={{ position: "relative", marginBottom: 15 }}>
      <label style={{ fontSize: 12, color: "#aaa", display:"block", marginBottom:5 }}>Agregar otro producto (Armazón / Accesorios)</label>
      <input 
        value={query} 
      ...
      {filteredProducts.length > 0 && (
        <div style={{ position: "absolute", zIndex: 50, background: "#333", border: "1px solid #555", width: "100%", borderRadius: 6, marginTop: 4, overflow: "hidden" }}>
